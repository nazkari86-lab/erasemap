...
// 01 — cover
{
  ...
  notes(s, "Начните с простой мысли: ответ DELETE 200 OK подтверждает только команду, но не исчезновение копий и производных. EraSeMap проверяет весь путь данных.");
}
...
{
  const s = pptx.addSlide(); s.background = { color: C.bg };
  title(s, "Вердикт", "Система должна уметь сказать: INCOMPLETE", 4, "Если путь ещё активен, EraSeMap не объявляет удаление завершённым.");
...
// 07 — efficiency evidence
{
  ...
  notes(s, "Сравнение сделано после одинаковых обязательных проверок. Это ограниченная лабораторная задача, поэтому цифры нельзя автоматически переносить на рабочую систему.");
}
...
// 08 — hidden path
{
  ...
  notes(s, "Активная проба выбирает следующий тест так, чтобы разделить оставшиеся гипотезы. Проверка на новых семействах топологий показывает перенос внутри эксперимента, но не доказывает готовность рабочей системы.");
}
...
// 09 — system comparison
{
  ...
  notes(s, "Не скрывайте отрицательный результат по модели. Основной результат проекта — проверка удаления в хранилищах; модельная ветка оценивается отдельно. Четыре строки на слайде нельзя складывать в один рейтинг.");
}
...
// 12 — novelty
{
  ...
  addText(s, "Сейчас: воспроизводимый эксперимент и ограниченные внешние данные.\nДальше нужна независимая скрытая проверка.", 6.52, 5.62, 5.52, 0.4, 10.7, C.muted, { align: "center" });
