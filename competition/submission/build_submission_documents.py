...
ABSTRACT_RU = (
    ...
    "в биометрическом шаблоне и векторе; влияние — в обученной модели. В работе представлен EraSeMap — "
    "один алгоритм из трёх стадий. FIND находит зарегистрированные и ограниченно скрытые пути. ERASE "
    "выбирает минимальный достаточный набор физических действий и machine unlearning. PROVE выполняет "
    "временной повтор операций и разрешает сертификат только после прохождения всех обязательных каналов. "
    "В frozen transfer на 60 случаях EraSeMap дал 0 ложных COMPLETE против 5 у полного typed-аудита и "
    "45 у native service status. Активный поиск recovery-графа потребовал 7 проб против 13 у случайной "
    "стратегии и 49 у полного перебора. В 20 парных опытах с реальными локальными процессами targeted plan "
    "был в 17,64 раза быстрее rebuild-all и записал на 94,62% меньше байтов. Временная проверка обнаружила "
    "30 из 30 латентных рисков и после controls дала 0 из 30 возвратов. Быстрые unlearning-кандидаты на "
    "Qwen2.5-1.5B не прошли все заранее зафиксированные пороги, поэтому точное переобучение сохранено как "
    "безопасный вариант. Результаты ограничены зарегистрированными топологиями и переходами; промышленное "
    "внедрение и независимый скрытый эксперимент не заявляются."
)
...
ABSTRACT_EN = (
    ...
    "vectors; learned influence may remain in a trained model. This work presents EraSeMap, one algorithm "
    "with three stages. FIND locates registered and bounded-hidden paths. ERASE selects a minimum sufficient "
    "set of physical actions and machine unlearning. PROVE performs temporal replay and permits a certificate "
    "only after every mandatory channel passes. In a frozen 60-case transfer, EraSeMap produced 0 false "
    "COMPLETE decisions versus 5 for full typed audit and 45 for native service status. Active recovery-graph "
    "search used 7 probes versus 13 for random testing and 49 for exhaustive testing. In 20 paired real-process "
    "trials, targeted erasure was 17.64 times faster and wrote 94.62% fewer bytes than rebuild-all. Temporal "
    "verification detected 30 of 30 latent risks and produced 0 of 30 post-control recurrences. Fast unlearning "
    "candidates on Qwen2.5-1.5B failed all preregistered gates jointly, so exact retraining remains the safe "
    "fallback. Results are bounded to registered topology and transition envelopes; production deployment and "
    "an independent hidden result are not claimed."
)
...
def build_theses(path: Path) -> None:
    ...
    add_section(
        doc,
        "Цель и гипотеза",
        "Цель — разработать один проверяемый алгоритм удаления персональных данных, который учитывает "
    ...
    add_section(
        doc,
        "Метод",
        "FIND строит типизированную карту данных и безопасными пробами уточняет ограниченное множество "
        "скрытых recovery-путей. ERASE выбирает минимальный достаточный набор разрешённых действий; "
        "machine unlearning является обязательным действием модельной ветки и сравнивается с точным "
        "переобучением. PROVE повторяет зарегистрированные будущие операции и выдаёт сертификат только "
    ...
    add_section(
        doc,
        "Основные результаты",
        "На 60 frozen случаях EraSeMap получил 0 ложных COMPLETE против 5 у полного typed-аудита и 45 у "
        "native status. Активный поиск использовал 7 проб против 13 у случайной стратегии и 49 у полного "
        "перебора. В 20 парных real-process опытах targeted ERASE был в 17,64 раза быстрее rebuild-all и "
        "записал на 94,62% меньше байтов. PROVE обнаружил 30 из 30 латентных рисков и после controls дал "
        "0 из 30 возвратов. Exact solvers совпали с exhaustive oracle в 3072 из 3072 и 16 384 из 16 384 "
        "конфигураций. Быстрые Qwen-unlearning кандидаты не прошли все gates; отрицательный результат "
        "сохранён, а точное переобучение оставлено безопасным fallback.",
    )
    add_section(
        doc,
        "Новизна и практическая значимость",
        "Новизна заключается не в изобретении lineage, set cover или machine unlearning по отдельности, "
        "а в одном fail-closed правиле решения: ни одна локальная квитанция не может самостоятельно создать "
        "положительный verdict. Подход применим к биометрии, банковским системам, электронным услугам, "
...
def build_supervisor_review(path: Path) -> None:
    ...
    add_section(
        ...
        "Код, протоколы и агрегированные результаты опубликованы в воспроизводимом репозитории. Точные "
        "решатели сопоставлены с exhaustive oracle, а ключевые ограничения указаны явно. Перед подписанием "
        "руководитель должен самостоятельно проверить соответствие этого описания фактическому вкладу автора "
        "и результатам, представленным в работе.",
    )
    add_section(
        doc,
        "Недостатки и рекомендации",
        "Основные ограничения — отсутствие промышленного пилота и завершённого независимого скрытого "
        "эксперимента, а также отрицательный результат быстрых unlearning-кандидатов. Рекомендуется провести "
    ...
    add_section(
        ...
        "Работа имеет исследовательскую и практическую ценность, содержит собственную программную реализацию, "
        "формализуемый алгоритм, измеряемые результаты и честно обозначенные границы применимости. После "
