...
{
  const s = pptx.addSlide(); s.background = { color: C.bg };
  title(s, "Механизм", "Как EraSeMap превращает удаление в доказательство", 5, "Каждый переход оставляет подтверждение, пробу, повторную проверку или сертификат.");
  const items = [
    ["Реестр", "что известно", C.blue], ["Пробы", "что отвечает", C.teal], ["План", "что сделать", C.ochre], ["Повтор", "что будет потом", C.coral], ["Сертификат", "что доказано", C.lilac],
  ];
  ...
  addText(s, "COMPLETE  ⇔  P ∧ D ∧ T", 1.55, 5.55, 4.3, 0.2, 16, C.teal, { fontFace: F_HEAD, bold: true, align: "center" });
  rounded(s, 6.63, 5.34, 5.35, 0.65, C.sage2, C.teal, 0.2, 0);
  addText(s, "Сертификат показывает границу знаний", 6.9, 5.55, 4.8, 0.2, 12.7, C.ink, { bold: true, align: "center" });
  addText(s, "Если путь не зарегистрирован или не проверен — verdict остаётся ограниченным.", 1.1, 6.44, 11.2, 0.25, 12, C.muted, { align: "center" });
  notes(s, "Здесь не нужно перечислять код. Покажите цепочку доказательства: реестр — проба — план — replay — сертификат.");
}
