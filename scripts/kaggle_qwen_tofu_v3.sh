...
if [[ "$action" == "submit" ]]; then
  if [[ -n "$(git -C "$project_root" status --porcelain)" ]]; then
    echo "Refusing to freeze v3 from a dirty worktree." >&2
    exit 2
  fi
  revision="$(git -C "$project_root" rev-parse HEAD)"
  source_temp="$(mktemp -d "${TMPDIR:-/tmp}/erasemap-kaggle-v3-source.XXXXXX")"
  kernel_temp="$(mktemp -d "${TMPDIR:-/tmp}/erasemap-kaggle-v3-kernel.XXXXXX")"
  cleanup_submit_temp() {
...
if [[ "$action" == "collect" ]]; then
  destination="$project_root/outputs/qwen-tofu-kaggle-v3"
  if [[ -e "$destination" ]]; then
    echo "Refusing to overwrite existing result: $destination" >&2
    exit 2
  fi
  download_temp="$(mktemp -d "${TMPDIR:-/tmp}/erasemap-kaggle-v3-output.XXXXXX")"
  ...
  "$kaggle_python" "$project_root/scripts/download_kaggle_kernel_output.py" \
    ...
    --prefix "qwen-tofu-v3/"
  source_result="$download_temp/qwen-tofu-v3"
  if [[ ! -f "$source_result/summary.json" ]]; then
    echo "Completed output does not contain qwen-tofu-v3/summary.json" >&2
    exit 1
  fi
  mkdir -p "$destination"
  cp -R "$source_result/." "$destination/"
  PYTHONPATH="$project_root/src:$project_root" \
    "$project_root/.venv/bin/python" "$project_root/scripts/verify_qwen_tofu_kaggle_v3.py" \
    --protocol "$project_root/benchmark/qwen-tofu-kaggle-v3.json" \
    --result "$destination"
  exit 0
fi
