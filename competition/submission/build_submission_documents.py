...
def build_theses(path: Path) -> None:
    ...
    add_section(
        doc,
        "Проблема",
        "Локальный статус «удалено» подтверждает только выполнение операции в одном компоненте, но не исчезновение копий, "
    ...
    add_section(
        ...
        "что проверять полный путь удаления безопаснее, чем доверять локальному статусу, а целевой план "
...
def build_supervisor_review(path: Path) -> None:
    ...
    add_section(
        doc,
        "Актуальность",
        "Я считаю тему работы своевременной и практически важной. В современной системе обычная процедура удаления "
        "часто затрагивает только основную запись, но не отвечает на вопрос, остались ли копии в кэше, реплике, "
        "индексе, резервной копии или в производной модели. Автор рассматривает эту проблему как единый путь "
        "данных и предлагает проверять не только сам факт удаления, но и отсутствие последующего восстановления. "
        "Такой подход может быть полезен для биометрических, банковских, государственных и образовательных систем.",
    )
    add_section(
        doc,
        "Личный вклад автора",
        "Дулат самостоятельно сформулировал исследовательский вопрос и разработал алгоритм FIND–ERASE–PROVE. "
        "Он реализовал программный прототип, подготовил сценарии проверок, провёл эксперименты, сравнил результаты "
        "с базовыми стратегиями и оформил воспроизводимый набор материалов. Отдельно отмечу, что автор не скрыл "
        "отрицательный результат быстрых методов machine unlearning: это показывает ответственное отношение к эксперименту. "
        "В работе ясно отделены результаты, ограничения и задачи дальнейшего исследования.",
    )
    add_section(
        doc,
        "Достоверность и самостоятельность",
        "Работа содержит код, зафиксированные протоколы и сводные результаты, поэтому основные выводы можно проверить "
        "повторным запуском. Для небольших задач автор сопоставил точные решатели с полным перебором и указал границы, "
        "в которых действуют его выводы. Перед подписанием я лично сверю формулировки с фактическим вкладом автора "
        "и представленными материалами.",
    )
    add_section(
        doc,
        "Недостатки и рекомендации",
        "Работа пока проверена на зарегистрированных сценариях, поэтому её нельзя считать доказательством готовности "
        "к промышленному внедрению. Независимый скрытый эксперимент и пилот на инфраструктуре организации ещё не завершены. "
        "В дальнейшем рекомендую провести такую проверку на заранее скрытых топологиях и, только при наличии разрешения, "
        "на обезличенной тестовой инфраструктуре. Отрицательный результат быстрых методов удаления влияния также следует "
        "сохранить как важную границу применимости, а не заменять его более удобной формулировкой.",
    )
    add_section(
        doc,
        "Заключение руководителя",
        "По итогам ознакомления считаю, что работа имеет исследовательскую и практическую ценность для секции "
        "«Информатика». В ней есть самостоятельная программная реализация, понятное описание алгоритма, измеряемые "
        "результаты и честно обозначенные ограничения. После моей личной проверки текста, фактического вклада автора "
        "и окончательного оформления работу можно рекомендовать к участию в конкурсе.",
    )
    doc.add_paragraph()
    for text in (
